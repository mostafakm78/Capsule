  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <div className="flex flex-col mt-12 lg:px-0 px-2 lg:flex-row lg:items-center justify-center w-full" aria-label={`Last capsules of user ${userId?.name ?? ''}`}>
          ...
          <div className="relative overflow-visible w-full lg:px-4 mx-auto max-w-[400px] md:max-w-3xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl min-h-[400px]" role="region" aria-label={`Carousel of last capsules by ${userId?.name ?? ''}`}>
                  ...
                  <SwiperSlide key={item._id} className="!w-[276px] shrink-0 pt-10" aria-label={`Capsule titled ${item.title}`}>
                    ...
                    >
                      <CardHeader className="px-0">
                        ...
                        <Image className="object-cover w-full h-[130px] mb-2 p-0.5 rounded-t-md rounded-b-sm shadow-secondary/30" src={item.image ? `http://localhost:8080/images/${item.image}` : '/images/def.jpg'} alt={`${item.title} image`} width={500} height={500} priority unoptimized />
