...
export default function LastRelatedCapsules({ singleCapsule }: Props) {
  ...
  if (singleCapsule === undefined) return <div>در حال بارگزاری</div>;

  const getId = (v: IdLike): string | undefined => (typeof v === 'string' ? v : v?._id);

  const currentCatId = getId(singleCapsule?.categoryItem);

  const related = (capsules ?? [])
    .filter((c) => getId(c.categoryItem) === currentCatId)
    .filter((c) => c._id !== singleCapsule?._id);
  ...
  return (
    <>
      ...
      <div className="relative overflow-visible w-full lg:px-4 mx-auto max-w-[400px] md:max-w-3xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl min-h-[400px]">
        {related && related.length <= 0 && <div className="h-[300px] w-full flex items-center justify-center border border-foreground/50 rounded-lg mt-2 text-3xl">کپسولی ساخته نشده !</div>}
              ...
              <SwiperSlide key={item._id} className="!w-[276px] shrink-0 pt-10">
                <Card key={item._id} className="lg:col-span-6 xl:col-span-4 flex flex-col relative bg-white py-0 hover:shadow-2xl hover:scale-[101%] hover:shadow-foreground/20 duration-300 dark:bg-slate-900 h-[420px] border-none shadow-sm ring-2 ring-primary/20">
                  <CardHeader className="px-0">
                    ...
                    <Image
                      className="object-cover w-full h-[130px] mb-2 p-0.5 rounded-t-md rounded-b-sm shadow-secondary/30"
