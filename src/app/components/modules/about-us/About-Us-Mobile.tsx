...
export default function AboutUsMobile() {
  // Animates the "sec-one" elements when they enter viewport on mobile
  useGSAP(() => {
    gsap.fromTo(
      '.sec-one',
      { opacity: 0, y: -50 },
      {
        ...
        stagger: 0.3,
        y: 0,
        scrollTrigger: { trigger: '.sec-one', start: 'top center', once: true },
      }
    );
  }, []);

  return (
    // Root section landmark for the mobile About Us page
    <section className="dotted block lg:hidden my-6" role="region" aria-label="درباره ما (موبایل)">
      {/* Page wrapper: contains all content blocks */}
      <div className="bg-background relative px-4 md:px-6 lg:px-10 gap-10 flex flex-col items-center justify-center">
        {/* Story block: region identified by its heading */}
        <section className="flex flex-col items-center gap-6" role="region" aria-labelledby="about-story-title">
          {/* Local header for the story section (title only) */}
          <header>
            <h3 id="about-story-title" className="sec-one text-5xl font-kalmeh text-foreground">
              داستان آغاز
            </h3>
          </header>
          ...
          {/* Decorative SVG divider (presentation only; hidden from screen readers) */}
          <svg className="sec-one text-foreground/70 h-[70px]" width="98" height="129" viewBox="0 0 98 129" fill="none" xmlns="http://www.w3.org/2000/svg" role="presentation" aria-hidden="true">
            <path
        ...
        {/* Mission card: region labeled by its heading */}
        <section className="flex flex-col items-center rounded-xl p-4 gap-6" role="region" aria-labelledby="mission-title">
          {/* Local header for mission section */}
          <header>
            <h3 id="mission-title" className="text-3xl font-kalmeh self-start text-foreground">
              مأموریت ما
            </h3>
          </header>
          {/* Decorative separator */}
          <Separator className="w-full bg-foreground/20" aria-hidden="true" />
        ...
        {/* Privacy card: region labeled by its heading */}
        <section className="flex flex-col items-center rounded-xl p-4 gap-6" role="region" aria-labelledby="privacy-title">
          <header>
            <h3 id="privacy-title" className="text-3xl font-kalmeh self-start text-foreground">
              امنیت و حریم خصوصی
            </h3>
          </header>
          <Separator className="w-full bg-foreground/20" aria-hidden="true" />
        ...
        {/* For now & future card */}
        <section className="flex flex-col items-center rounded-xl p-4 gap-6" role="region" aria-labelledby="future-title">
          <header>
            <h3 id="future-title" className="text-3xl font-kalmeh self-start text-foreground">
              {' '}
              برای حال، برای آینده
            </h3>
          </header>
        ...
        {/* With users card */}
        <section className="flex flex-col items-center rounded-xl p-4 gap-6" role="region" aria-labelledby="users-title">
          <header>
            <h3 id="users-title" className="text-3xl font-kalmeh self-start text-foreground">
              همراهی با کاربران
            </h3>
          </header>
          <Separator className="w-full bg-foreground/20" aria-hidden="true" />
        ...
        {/* Vision card */}
        <section className="flex flex-col items-center rounded-xl p-4 gap-6" role="region" aria-labelledby="vision-title">
          <header>
            <h3 id="vision-title" className="text-3xl font-kalmeh self-start text-foreground">
              چشم‌انداز
            </h3>
          </header>
          <Separator className="w-full bg-foreground/20" aria-hidden="true" />
