...
export default function AboutUsDesktop() {
  // Ref to the timeline SVG path used for the draw-on-scroll effect
  ...
  useGSAP(() => {
    ...
    // Prepare the stroke dash array for large screens to enable the draw effect
    ...
    // Animate the path drawing on scroll (desktop only)
    if (window.innerWidth > 1024) {
      gsap.fromTo(
        pathRef.current,
        {
    ...
    // Staggered reveal animations for the content cards
    gsap.fromTo('.box-one', { opacity: 0, x: 200 }, { opacity: 1, x: 0, delay: 0.3, duration: 0.3, scrollTrigger: { trigger: '.box-one', start: 'top 80%' } });
  ...
  return (
    // Page section landmark for the About page (localized Persian label)
    <section className="dotted lg:block hidden my-16" aria-label="درباره ما" role="region">
      {/* Layout wrapper for the timeline + content cards */}
      <div className="py-10 bg-background relative px-10 gap-56 flex flex-col items-center justify-center">
        {/* Decorative background path (hidden from assistive tech) */}
        <svg className="absolute flex items-center justify-center h-full w-full top-0 left-1/2 -translate-x-1/2 pointer-events-none text-foreground/50 z-[5]" width="3000" height="1440" viewBox="0 0 1440 3000" fill="none" xmlns="http://www.w3.org/2000/svg" role="presentation" aria-hidden="true">
          <path
            ref={pathRef}
        ...
        {/* First content block: Story (region labeled by its heading) */}
        <div className="flex w-10/12 z-20 items-center justify-center gap-20">
          {/* Self-contained content card with heading and paragraph */}
          <section className="box-one flex shadow-lg flex-col items-center bg-accent rounded-xl p-4 gap-6" role="region" aria-labelledby="story-header">
            {/* Card title acts as the accessible name via aria-labelledby */}
          ...
          {/* Brand image placed as a figure (semantic media grouping) */}
          <figure className="w-1/2 lg:h-[200px] xl:h-[300px] block transform rotate-90">
            {/* Decorative/branding image; kept aria-hidden to avoid noise for SRs */}
            <Image src="/images/Logo.png" alt="لوگوی کپسول" width={300} height={300} aria-hidden="true" />
          </figure>
        </div>

        {/* Second content block: Mission */}
        <div className="flex items-center justify-between w-10/12 gap-6 z-20">
          <div className="w-1/2 block"></div>
          <section className="box-two flex flex-col shadow-lg items-center bg-accent rounded-xl p-4 gap-6" role="region" aria-labelledby="mission-header">
        ...
        {/* Third content block: Privacy */}
        <div className="flex items-center justify-between w-10/12 gap-6 z-20">
          <section className="box-three flex flex-col shadow-lg items-center bg-accent rounded-xl p-4 gap-6" role="region" aria-labelledby="privacy-header">
        ...
        {/* Fourth content block: For now & future */}
        <div className="flex items-center justify-between w-10/12 gap-6 z-20">
          <div className="w-1/2 block" />
          <section className="box-four flex flex-col shadow-lg items-center bg-accent rounded-xl p-4 gap-6" role="region" aria-labelledby="future-header">
        ...
        {/* Fifth content block: With users */}
        <div className="flex items-center justify-between w-10/12 gap-6 z-20">
          <section className="box-five flex flex-col shadow-lg items-center bg-accent rounded-xl p-4 gap-6" role="region" aria-labelledby="users-header">
        ...
        {/* Sixth content block: Vision */}
        <div className="flex items-center justify-center w-10/12 gap-6 z-20">
          <section className="box-six flex flex-col shadow-lg items-center bg-accent rounded-xl p-4 gap-6" role="region" aria-labelledby="vision-header">
