          ...
          <div className="flex flex-col gap-3">
            {notifications.map((item) => (
              <div key={item._id} className="flex flex-col">
                ...
                <span className="text-foreground/50 text-xs uppercase">نوع پیام : {item.type === 'alert' ? 'هشدار' : item.type === 'message' ? 'پیام' : item.type === 'news' ? 'اخبار' : item.type === 'system' ? 'سیستم' : 'پیام'}</span>
                <Separator className="w-full bg-foreground/20 my-2" />
              </div>
            ))}
          </div>
        )}
