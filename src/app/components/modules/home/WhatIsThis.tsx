...
export default function WhatIsThis() {
  useGSAP(() => {
    // Animate the "text" items (three feature blurbs) on scroll from right to left
    gsap.fromTo(
      '.text',
      { opacity: 0, x: 50 },
      {
    ...
    // Animate the arrow graphic on scroll from left to right
    gsap.fromTo(
      '.arrow',
      { opacity: 0, x: -50 },
      {
      ...
      <section className="flex lg:flex-row flex-col gap-4 lg:gap-0 w-full xl:w-2/3 lg:w-3/4 px-10 justify-between">
        ...
        <article className="text flex items-center justify-center gap-2 text-xl lg:text-2xl">
          ...
          <span className="relative z-[2] after:content-[''] after:absolute after:w-[20px] after:h-[20px] lg:after:h-[30px] lg:after:w-[30px] lg:after:top-2 after:top-1 lg:after:left-3 after:left-2 after:rounded-full after:bg-purple-500/25">
            <TbLibraryPhoto className="text-purple-500" />
          </span>
          {/* Short label for the feature */}
          <h4 className="text-foreground/70">عکس اضافه کن</h4>
        </article>

        {/* Feature 2: Set time */}
        <article className="text flex items-center justify-center gap-2 text-xl lg:text-2xl">
          <span className="relative z-[2] after:content-[''] after:absolute after:w-[20px] after:h-[20px] lg:after:h-[30px] lg:after:w-[30px] lg:after:top-2 after:top-1 lg:after:left-3 after:left-2 after:rounded-full after:bg-red-500/25">
            <IoIosTime className="text-red-500" />
          </span>
          <h4 className="text-foreground/70">زمان مشخص کن</h4>
        </article>

        {/* Feature 3: Send encrypted */}
        <article className="text flex items-center justify-center gap-2 text-xl lg:text-2xl">
          ...
          <h4 className="text-foreground/70">رمزنگاری شده ارسال کن</h4>
        </article>
      </section>
